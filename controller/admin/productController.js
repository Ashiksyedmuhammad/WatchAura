...
const addProduct = async (req, res) => {
  try {
    ...
    const imagePromises = imageFields.flat().map(async (file) => {
      ...
      try {
        await sharp(imagePath).resize(200, 200).toFile(outputImagePath);
...
const editProduct = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "Error editing product: " + error.message,
      });
  }
};
...
const updateStatus = async (req, res) => {
  try {
    ...
    const checkingOrder = await Orders.findById(orderid);
    let num = 0;

    checkingOrder.items.forEach((x) => {
      if (x.status == "Delivered") {
        num++;
      }
    });
    if(num==checkingOrder.items.length){
